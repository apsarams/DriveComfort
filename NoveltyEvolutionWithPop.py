# This script creates a test suite, which contains roads, where the distance between each curve profile
...
finalPath = 'C:/Users/apsara.murali-simha/Tests_Final/Random/Random/test_01'
...
def runTest(road):
    ...
    try:
        ...
        try:
            ...
            export_test_gen(plot_path, plot_path, newTest, render=True)
...
def crossParents(mom, dad):
    ...
    try:
        ...
        plot_path = c.rg.get_plots_path()
        port = getPort()
        runner = TestRunner(newTest, "asfault", 'localhost', port, False)
        try:
            execution = func_timeout(90, runner.run)
            export_test_exec(final_path, final_path, newTest)
            export_test_gen(plot_path, plot_path, newTest, render=True)
...
toolbox.register("individual", tools.initRepeat, creator.Individual,toolbox.getInd, n=1)
