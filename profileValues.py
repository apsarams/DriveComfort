...
from src.asfault.experiments import *
from src.asfault.tests import *
...
def runScenario(scenario):
    setup_logging()
    bng = BeamNGpy('localhost', 64256, home='C:/Users/apsara.murali-simha/beamng_research/trunk')
...
def getRandomPopulation(budget):
    ...
    ev = EvolutionConfig('C:/Users/apsara.murali-simha/.asfaultenv/cfg/evolution.json')
    con = ev.get_default()

    # setting up config for every module
    DEFAULT_ENV = os.path.join(str(Path.home()), '.asfaultenv')
    c.load_configuration(DEFAULT_ENV)
    final_path = c.rg.get_final_path()
    plot_path = c.rg.get_plots_path()
    ...
    while (count < int(budget)):
        ...
        export_test_gen(plot_path, plot_path, test, render=True)
...
def getRandomPopulationWithoutOOB(budget):
    ...
    while (count < int(budget)):
        ...
        # generates execution states
        runner = TestRunner(test, "asfault", 'localhost', port, False)
        execution = runner.run()
...
def getOobTuples(dic):
    ...
    if seg_oob_count:
        tup = (dic['execution']['oob_speeds'], [dic['execution']['seg_oob_count']], [dic['execution']['oob_force']])
        resList.append(tup)
    return resList
